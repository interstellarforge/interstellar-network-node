...
The Toolbox can apply the tailnet Grant through the Tailscale API, but only from its own menu entry and never as part of install. The policy file is HuJSON and usually carries the operator's comments, so the grant is inserted textually rather than parsed and re-serialized; a rewrite would silently delete every comment. When the insertion point cannot be identified with certainty the Toolbox refuses and prints the snippet. Writes are preceded by the API's own `/acl/validate` call and carry `If-Match` with the ETag from the read, so a concurrent admin-console edit fails the write instead of losing that edit. The credential needs the `policy_file` scope, is passed by environment rather than argv, and is never persisted.
